...
def generate_doc_id_from_metadata(url, date_doc):
    print(f"voila date doc!!!!! {date_doc} ")
...
def remove_footer_metadata(text):
    ...
    """
    return re.sub(
        r"Liens\s*:\s*https://www\.ejustice\.just\.fgov\.be/cgi/article\.pl\?.*?(Haut de la page|Copier le lien|"
        r"Image du Moniteur belge.*?)$",
        "",
        text,
        flags=re.DOTALL
    )
...
def extract_date_after_birthday(texte_html):
    ...
    patterns = [
        r"succession\s+vacante\s+de\s+(?:M(?:onsieur)?|Madame)?\.?\s+[A-ZÉÈÊÀÂa-zéèêàâçëïüö'\-]"
        r"+\s*,\s+[A-ZÉÈÊÀÂa-zéèêàâçëïüö'\-]+\s*,\s+né\s+à\s+[^\d\n]{1,50}?\s+le\s+(\d{1,2}\s+\w+\s+\d{4})",
